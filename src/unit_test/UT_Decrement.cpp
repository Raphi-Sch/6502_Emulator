...
bool decrement(CPU& cpu, Memory& mem, byte instruction){
    bool valid = true;
    ...
    if(!expected_eq(mem.read(addr), 0x02, instructionName, "Result")) valid = false;
...
int * run_decrement(CPU& cpu, Memory& mem){
    static int result[2];

    result[0]++; if(decrement(cpu, mem, CPU::INS_DEC_ZP)) result[1]++;
    result[0]++; if(decrement(cpu, mem, CPU::INS_DEC_ZPX)) result[1]++;
    result[0]++; if(decrement(cpu, mem, CPU::INS_DEC_ABS)) result[1]++;
    result[0]++; if(decrement(cpu, mem, CPU::INS_DEC_ABSX)) result[1]++;
